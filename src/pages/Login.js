...
const responseFacebook = (response) => {
  ...
  if (response.status === 'connected') {
    ...
    let data={userAccessToken:accessToken,social_type:1}
    this.props.fetchSignin(data, (response) => {
...
const googleLoginSucess = (response) => {
  console.log(response);
  console.log("response from google", response);
  if (response.status === 'connected') {
    const { accessToken } = response.accessToken;
    let data={userAccessToken:accessToken,social_type:2}
    this.props.fetchSignin(data, (response) => {
      console.log("Response from Signin", response)
      let {data} = response;
      if (data.success === true) {
              ...
              <div className="col-lg-4 mx-auto">
                <div className="auth-form-light text-left p-5">
                  ...
                  <form className="pt-5" autoComplete="off" onSubmit={this.login}>
                    ...
                    <div className="other-option" style={{textAlign:"center"}}>
                      <p>Or sign in with</p>
                      <div className="sign_action_btn">
                        <div className="fb-btn action-block">
                          <FacebookLogin 
                               appId="268877253976085" 
                               autoLoad={false}
                               callback={responseFacebook}
                               render={renderProps => (
                      <button className="btn btn-primary form_coman_lgbtn blue-bg"  onClick={renderProps.onClick}> <i className="fa fa-facebook" aria-hidden="true" />Facebook</button> )}/>
                        </div>
                        <div className="fb-btn action-block" style={{marginTop:"10px"}}>
                          <GoogleLogin
                          clientId="AIzaSyAklbNRBMGEEJYfQhCtNiuo6JiKOoh2w2M.apps.googleusercontent.com"
                          buttonText="Login"
                          onSuccess={googleLoginSucess}
                          onFailure={googleLoginFail}
                               render={renderProps => (
                      <button className="btn btn-primary form_coman_lgbtn blue-bg"  onClick={renderProps.onClick}> <i className="fa fa-google" aria-hidden="true" />Google</button> )}/>
                        </div>
                        {/* <div className="fb-btn action-block">  
                       <GoogleLogin
                          clientId="AIzaSyAklbNRBMGEEJYfQhCtNiuo6JiKOoh2w2M.apps.googleusercontent.com"
                          ...
                          onSuccess={googleLoginSucess}
                          onFailure={googleLoginFail}
                          className="btn btn-primary form_coman_lgbtn blue-bg"
                        />
